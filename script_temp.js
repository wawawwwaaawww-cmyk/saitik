document.addEventListener('DOMContentLoaded', () => {
    ...
    const createApplicationModal = () => {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                <span class="close-button" role="button" tabindex="0" aria-label="Закрыть окно"></span>
    ...
    if (applicationModal && closeButton) {
        ...
        document.addEventListener('click', (event) => {
            ...
            const closeTarget = event.target.closest('.close-button');
            if (closeTarget && applicationModal.contains(closeTarget)) {
                event.preventDefault();
                closeApplicationModal();
            }
        });
